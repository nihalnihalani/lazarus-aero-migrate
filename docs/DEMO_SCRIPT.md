...
| **Prove (the heart)** | It compiles & runs the **original COBOL with real GnuCOBOL**, captures its real output, generates equivalence tests, runs `pytest`. RED if a rounding idiom diverges. | "Here's what matters: it does **not** grade its own homework. It diffs against the real COBOL's actual output as the source of truth. Watch the cases." |
...
- [ ] **Pre-warm + reuse the `environment_id`** so 0:00 isn't a cold start and GnuCOBOL is already installed (no live network needed on stage). Verify `cobc --version` runs after reconnect.
...
- **"Why does it take a few minutes?"** → "Because it's real. It provisions a live sandbox, installs a real COBOL compiler, runs the original program for ground truth, and proves equivalence. The runtime is the proof it isn't a canned animation."
- **"How do you know the translation is correct?"** → "We compile and run the original COBOL with real GnuCOBOL in the sandbox and assert byte-for-byte equivalence against its output. The oracle is the real program's output, not our own tests."
