...
| **0:35–0:55** | Agent writes `payroll.py`. Then compiles & runs the **original COBOL with real GnuCOBOL** (pre-installed in the reused sandbox env), captures its real output, generates equivalence tests, runs `pytest`. Terminal goes **RED** (3 failures). | "It doesn't grade its own homework — it runs the *real* COBOL as the source of truth, and diffs against it. Right now they don't match." |
