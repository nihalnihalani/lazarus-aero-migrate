...
- **Google DeepMind (technical / CTO judges):** will probe correctness, where it breaks, scalability. → Lead with the **GnuCOBOL differential oracle** (real correctness, not self-graded tests) and the honest single-agent architecture. Be ready for "is this actually Managed Agents?" — yes, code execution + persistent files + `SKILL.md`.
- **AI Futures Fund deal team (investor judges):** 1st place includes a 30-min call with them. They ask "could this be a company?" → Frame LAZARUS as eating the **$30B+ legacy-modernization services budget** with a defensible moat (accumulated dialect skills + regulated-trust + a translation/equivalence corpus).
...
1. **Problem** — NJ 2020: the governor begged for COBOL programmers; 575k claims backlogged.
...
- [x] Files (oracle binary, forged skills, migrated output) persist across turns.
