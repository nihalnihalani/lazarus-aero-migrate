...
| **Cross-run skill library** — bank a forged `SKILL.md` to disk and re-mount it on the next run so skills accumulate | re-register on change (off by default) | **Cross-run live-verified end-to-end** (pinned SDK 2.6.0): forge→bank→re-register→fresh-run discovery in one run — run A banks a `SKILL.md`, the fingerprint re-registers, and a *separate* fresh run B emits a sentinel found only in run A's skill ([`docs/EVIDENCE.md`](docs/EVIDENCE.md) §3). Live testing also caught + fixed a parser bug that mis-banked the migrated module (`34910c2`, regression-guarded). |
