...
| **Cross-run skill library** — bank a forged `SKILL.md` to disk and re-mount it on the next run so skills accumulate | re-register on change (off by default) | **Discovery live-verified** (pinned SDK 2.6.0, sentinel token); banks a real forged `SKILL.md` on clean output. Live testing caught the parser mis-banking the migrated module on real `migrate()` output — **found, fixed (`34910c2`), regression-guarded**. A clean end-to-end `migrate()` capture is follow-up #9. |
